...
export default function AdminLayout({
  ...
  return (
    <div className='relative min-h-screen w-full flex bg-[#0f172a] font-sans text-white overflow-hidden'>
      ...
      {/* --- SIDEBAR --- */}
      <aside
        className={`fixed md:relative z-50 h-screen transition-all duration-300 ${isMobileMenuOpen ? 'left-0' : '-left-full md:left-0'} w-72 bg-white/5 backdrop-blur-3xl border-r border-white/10 p-6 flex flex-col`}
      >
        ...
        <nav className='flex-1 space-y-8 overflow-y-auto no-scrollbar pr-2'>
          {menuConfig.map((section) => (
            <div key={section.title} className='space-y-2'>
              <h3 className='px-4 text-[10px] uppercase tracking-[0.2em] text-slate-500 font-bold'>
                {section.title}
              </h3>
              {section.items.map((item: any) => (
                ...
                >
                  <span
                    className={
                      activeTab === item.name ? 'text-indigo-400' : 'group-hover:text-indigo-300'
                    }
                  >
                    {item.icon}
                  </span>
                  <div className='text-left'>
                    ...
                    <p className='text-[10px] text-slate-600 group-hover:text-slate-400 transition-colors whitespace-nowrap'>
                      {item.desc}
                    </p>
                  </div>
                </button>
        ...
        <header className='flex items-center justify-between p-6 bg-[#0f172a]/80 backdrop-blur-xl border-b border-white/10 sticky top-0 z-30'>
          ...
          <div className='flex items-center gap-6'>
            ...
            <div className='flex items-center gap-3 pl-6 border-l border-white/10'>
              ...
              <div className='w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-500 to-violet-500 flex items-center justify-center font-black shadow-lg'>
                AD
              </div>
            </div>
          </div>
